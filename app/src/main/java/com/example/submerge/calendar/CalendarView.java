//Had to change the package decleration to make it work correctly - Ethan
package com.example.submerge.calendar;
...
import com.example.submerge.calendar.exceptions.ErrorsMessages;
import com.example.submerge.calendar.exceptions.OutOfDateRangeException;
import com.example.submerge.calendar.extensions.CalendarViewPager;
import com.example.submerge.calendar.listeners.OnCalendarPageChangeListener;
import com.example.submerge.calendar.listeners.OnDayClickListener;
import com.example.submerge.calendar.utils.AppearanceUtils;
...
import static com.example.submerge.calendar.utils.CalendarProperties.FIRST_VISIBLE_PAGE;
 ...
 *
 * Editied By Ethan Wolfe on 4/27/2020
 * Source Code can be found at https://github.com/Applandeo/Material-Calendar-View
 * At first I tried to edit his class using polymorphism, but due to his constructers being private, it would not work correctly.
 * I have put a comment above all of the code I have modified
 */
...
public class CalendarView extends LinearLayout {
    ...
    private void setCalendarRowLayout() {
        if (mCalendarProperties.getEventsEnabled()) {
            mCalendarProperties.setItemLayoutResource(com.example.submerge.R.layout.calendar_view_day);
        } else {
...
//    //Ethan - Add a function to add an event day instead of having to recalculate every time
//    public void addEvent(EventDay eventDay) {
//        if (mCalendarProperties.getEventsEnabled()) {
//            mCalendarProperties.addEventDay(eventDay);
//            mCalendarPageAdapter.notifyDataSetChanged();
//        }
//    }
    ...
    public Calendar getFirstShowingDate() {
        Calendar calendar = (Calendar) mCalendarProperties.getFirstPageCalendarDate().clone();
        calendar.add(Calendar.MONTH, mViewPager.getCurrentItem());
        calendar.set(Calendar.DAY_OF_MONTH, 1);
